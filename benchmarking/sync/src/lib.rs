//! SyncLM — blocking sequential tool-calling baseline for the AsyncLM
//! benchmark.
//!
//! Same CML wire format as AsyncLM (`[CALL] <id> [HEAD] <code> [END]` →
//! `[INTR] <id> [HEAD] <result> [END]`) and the same parser/registry, but
//! every `[CALL]` **blocks** generation and runs to completion before the
//! next decode step. Wall-clock cost for k calls is `Σ wait_i`. No
...
fn default_system() -> String {
    "You are an assistant that uses sequential, blocking function calls to\n\
     ...
     - The runtime runs ONE [CALL] at a time. Generation BLOCKS at every\n\
       [CALL]'s closing [END] until the matching [INTR] frame arrives.\n\
     - Emit ONE [CALL] at a time. Wait for its [INTR] frame, then either\n\
       emit the next [CALL] or write the final natural-language answer.\n\
     - There is no [TRAP] frame. Do not emit one.\n\
...
async fn main(input: Input) -> Result<String> {
    ...
    println!(
        "[SyncLM] call={:?} head={:?} end={:?} trap={:?} intr={:?}",
        registry.call_ids,
        registry.head_ids,
        registry.end_ids,
        registry.trap_ids,
